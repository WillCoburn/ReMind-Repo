...
export const sendExportLink = onCall(
  ...
  async (req) => {
    ...
    try {
      ...
      if (!exists) {
        throw new HttpsError("not-found", "Export file not found. Upload may not have completed.");
      }
      ...
      if (!link) {
        // Ensure a download token exists on the file's metadata
        const [meta] = await file.getMetadata();
        const tokensRaw = meta?.metadata?.firebaseStorageDownloadTokens as string | undefined;
        let token =
          tokensRaw?.split(",").map((s) => s.trim()).filter(Boolean)[0] || randomUUID();
      ...
      if (!to) {
        throw new HttpsError("failed-precondition", "No phone number on file for user.");
      }
      ...
      // Surface typed HttpsError if thrown; otherwise wrap as internal
      if (e instanceof HttpsError) throw e;
      logger.error("[sendExportLink] failed", { uid, path, message: e?.message, stack: e?.stack });
