...
struct PhoneEntryScreen: View {
    ...
    var body: some View {
        GeometryReader { geo in
            let lift = keyboard.height(in: geo)

            ZStack(alignment: .bottom) {
                VStack(spacing: 0) {
                    topContent
                        .padding(.top, keyboard.isVisible ? 12 : 44)
                    ...
                    .padding(.horizontal, 24)
                    .padding(.top, keyboard.isVisible ? 16 : 28)

                    Spacer(minLength: keyboard.isVisible ? 6 : 0)
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity, alignment: .top)
                .padding(.bottom, keyboard.isVisible ? 8 : 0)
